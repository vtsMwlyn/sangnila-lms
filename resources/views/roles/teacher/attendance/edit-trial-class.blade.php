...
@section("content")
	<x-section-container>
		...
		<form action="{{ route('teacher.attendance.trial-class.update', [$course->id, $trial_class_attendance->id]) }}" method="post" id="store-trialclass-form">
			@csrf
            ...
            <div class="flex gap-5">
                <div class="mt-3 w-full md:w-1/2">
                    ...
                    <x-input type="date" name="attendance_date" id="attendance_date" class="w-full date-input" value="{{ old('attendance_date', Carbon\Carbon::parse($trial_class_attendance->attendance_date)->format('Y-m-d')) }}"/>
                </div>
                <div class="mt-3 w-full md:w-1/2">
            ...
            <div class="flex gap-5">
                <div class="mt-3 w-full md:w-1/2">
                    ...
                    <x-input type="time" name="start_time" id="start_time" class="w-full" value="{{ old('start_time', Carbon\Carbon::parse($trial_class_attendance->start_time)->format('H:i')) }}"/>
                </div>
                <div class="mt-3 w-full md:w-1/2">
                    <x-label class="mb-1" for="end_time">End Time<span class="text-red">*</span></x-label>
                    <x-input type="time" name="end_time" id="end_time" class="w-full" value="{{ old('end_time', Carbon\Carbon::parse($trial_class_attendance->end_time)->format('H:i')) }}"/>
                </div>
            </div>

            <div class="mt-4 flex flex-col w-full">
				<x-label for="attendance_detail">Learning Details<span class="text-red">*</span></x-label>
				<x-textarea rows="4" id="attendance_detail" class="w-full mt-1" type="text" name="attendance_detail" placeholder="Enter candidate learning details">{!! old('attendance_detail', $trial_class_attendance->attendance_detail) !!}</x-textarea>
			</div>
