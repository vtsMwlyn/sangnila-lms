...
@section("content")
	<x-section-container>
		<x-page-title>Delete Activity</x-page-title>

		<form method="POST" action="{{ route("teacher.mycourse.activity.destroy", $activity->id) }}" class="rounded-2xl p-5 bg-blue-800">
			@csrf
			@method('DELETE')
			<!-- Confirmation Text -->
			<div class="mb-6">
				<h1 class="text-xl font-semibold text-white text-center">
					...
					<span class="text-red-500 font-bold">{{ $activity->title }}</span>
					from
					<span class="text-green-500 font-bold">{{ $activity->topic->title }}</span>
					in
					<span class="text-green-500 font-bold">{{ $activity->topic->course->course_name }}</span>
					?
				</h1>
			</div>
