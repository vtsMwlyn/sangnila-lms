...
@section("content")
	<x-section-container>
		...
		<form method="POST" action="{{ route('admin.course.curriculum.topic.delete', [$course->id, $curriculum_topic->id]) }}" class="bg-blue-800 rounded-2xl py-5 px-10 mt-8">
			@csrf

			<!-- Confirmation Text -->
			<div class="mb-6">
				<h1 class="text-xl font-semibold text-white text-center">
					Are you sure you want to permanently delete topic <span class="text-yellow-500 font-bold">{{ $curriculum_topic->title }}</span> from
					<span class="text-red-500 font-bold">{{ $course->course_name }}</span>'s curriculum?
				</h1>
			</div>
